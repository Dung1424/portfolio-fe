    ...
    <div id="explore-grid" class="d-flex flex-wrap" data-aos="fade-up" data-aos-delay="200">
        ...
        <div class="top-categories mb-4">
            ...
            <div class="top-category-header d-flex justify-content-between align-items-center">
                ...
                <NuxtLink :to="{ name: 'Category' }" class="view-all" style="color: #007bff;">
                    View All &gt;
                </NuxtLink>
            </div>
...
<script>
import { sharedService } from '~/features/shared/services/shared.api.js'
import AddToGalleryModal from './AddToGalleryModal.vue'
import ReportPhotoModal from './ReportPhotoModal.vue'
import ReportGalleryModal from './ReportGalleryModal.vue'
    ...
    methods: {
        async fetchTopLikedPhotos() {
            try {
                ...
                const response = await sharedService.fetchTopLikedPhotos({ headers });
        ...
        async fetchTopUsers() {
            try {
                ...
                const response = await sharedService.fetchTopUsersWithPhotos({ headers });
        ...
        async fetchTopCategories() {
            try {
                const response = await sharedService.fetchTopCategories();
        ...
        async fetchTopLikedGalleries() {
            try {
                ...
                const response = await sharedService.fetchTopLikedGalleries({ headers });
