        ...
        >
          <template #bodyCell="{ column, record }">
            ...
            <template v-else-if="column.key === 'unlock'">
              <a-button size="small" type="primary" class="!rounded-lg !shadow-sm" title="Unlock" @click="confirmUnlock(record.id)">
                <i class="fa-solid fa-unlock" aria-hidden="true" />
              </a-button>
            </template>
...
const {
  ...
  fetch: async (q) => {
    const { data } = await usersAdminApi.usersInactive(q)
    return data
  }
})
...
const columns = [
  ...
  { title: '', key: 'view', width: 56, fixed: 'right' },
  { title: '', key: 'unlock', width: 72, fixed: 'right' }
]
...
async function confirmUnlock(userId) {
  const ok = await adminConfirm({
    title: 'Unlock this user?',
    content: 'They will be able to sign in again.',
    okText: 'Unlock',
    cancelText: 'Cancel'
  })
  if (ok) {
    await doUnlock(userId)
  }
}

async function doUnlock(userId) {
  try {
    await usersAdminApi.unlockUser(userId)
