...
export default {
    ...
    methods: {
        ...
        beforeUpload(file) {
            ...
            // Nếu là video → chặn lại
            if (isVideo) {
                Modal.error({
                    title: 'Invalid File Type',
                    content: 'Videos are not allowed. Please upload image files only.',
                });
                return Upload.LIST_IGNORE; // Chặn upload + không thêm vào fileList
            }
            ...
            return true; // Cho phép upload ảnh
        },
        addKeyword(keyword) {
