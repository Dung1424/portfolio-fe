...
import PhotoLikeGrid from './PhotoLikeGrid.vue'
import GalleryLikeGrid from './GalleryLikeGrid.vue'
            ...
            try {
                ...
                this.likedPhotos = response.data.data.map(like => ({
                    ...
                    userAvatar: like.photo?.user?.profile_picture
                        ? `${this.apiOrigin}${like.photo?.user?.profile_picture}`
            ...
            try {
                ...
                this.likedGalleries = response.data.data.map(like => ({
                    ...
                    userAvatar: like.gallery?.user?.profile_picture
                        ? `${this.apiOrigin}${like.gallery?.user?.profile_picture}`
                        : '/images/imageUserDefault.png',
                }));
            } catch {
                notification.error({
        ...
        async deleteLikePhoto(like) {
            Modal.confirm({
                ...
                onOk: async () => {
                    ...
                    try {
                        const response = await accountService.deleteLike(like.id, {
                            headers: { Authorization: `Bearer ${token}` },
                        });
